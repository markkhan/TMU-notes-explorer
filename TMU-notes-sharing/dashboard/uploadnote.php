...
echo "<script>alert('Attach a file');</script>";
    ...
    else if ($_FILES['file']['size'] <= 0 || $_FILES['file']['size'] > 50720000 )
    {
echo "<script>alert('file size is not proper');</script>";
        ...
        if(move_uploaded_file($_FILES['file']['tmp_name'], $folder.$notefile)) {
            $query = "INSERT INTO uploads(Course_code,file_name, file_description, file_type, Semester ,file_uploader, file_uploaded_to, file) VALUES ('$Course_code','$file_title' , '$file_description' , '$fileext','$Semester' , '$file_uploader' , '$file_uploaded_to' , '$notefile')";
...
?>
	...
	<div class="form-group">
        ...
        <div class="form-group">
        <label for="post_tags">Semester</label>
        <input type="text" name="Semester" class="form-control" placeholder="Eg: VIth" value="<?php if(isset($_POST['upload'])) {
            echo $Semester;  } ?>" required="" " style="text-transform:uppercase">

    <div class="form-group">
        <label for="post_tags">Course Code</label>
        <input type="text" name="Course_code" class="form-control" placeholder="Eg: BCA653" value="<?php if(isset($_POST['upload'])) {
            echo $Course_code;  } ?>" required="" " style="text-transform:uppercase">
	</div>
	 <div class="form-group">
        <label for="post_image">Select File</label><font color="brown"> (allowed file type: 'pdf','doc','ppt','txt','zip' | allowed maximum size: 50 mb ) </font>
